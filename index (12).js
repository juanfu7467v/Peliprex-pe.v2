...
/**
 * Realiza una búsqueda en la API externa Peliprex y devuelve los resultados.
 * Acepta parámetros: q, genre, year.
 * Devuelve un array vacío si ocurre algún error.
 */
async function fetchExternalSearch(params = {}) {
  try {
    ...
    console.error("❌ Error en búsqueda externa Peliprex:", err.message);
    return [];
  }
}
...
app.get("/nuevo", async (req, res) => {
  try {
    ...
    console.error("❌ Error al obtener catálogo desde API externa (/nuevo):", err.message);
    res.status(500).json({ error: "Error al obtener el catálogo de la API externa." });
  }
});
