...
function mergeExternalIntoPeliculasFile(externalMovies) {
  try {
    ...
    // Persistir en disco
    fs.writeFileSync(PELIS_FILE, JSON.stringify(localPeliculas, null, 2), "utf8");

    console.log(`💾 [peliculas.json] ${nuevas.length} película(s) nueva(s) guardadas. Total local: ${localPeliculas.length}`);
  } catch (err) {
...
async function loadExternalApiMovies() {
  try {
    ...
    // 💾 Guardar automáticamente las películas nuevas en peliculas.json
