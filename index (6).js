 ...
 * Las películas locales tienen prioridad.
 */
function combinarResultados(locales, externos) {
  const titulosLocales = new Set(
    locales.map(p => (p.titulo || "").toLowerCase().trim())
  );
  const externosFiltrados = (externos || []).filter(
    p => !titulosLocales.has((p.titulo || "").toLowerCase().trim())
  );
  return [...locales, ...externosFiltrados];
}

/**
 * Realiza una petición a la API externa de búsqueda y devuelve los resultados.
 * Nunca lanza excepción: en caso de error devuelve array vacío.
 */
...
// 🔎 1. BÚSQUEDA POR NOMBRE — Combina peliculas.json + API externa
...
// 🔎 3. BÚSQUEDA AVANZADA — Combina peliculas.json + API externa
app.get("/buscar", async (req, res) => {
  ...
  // Construir parámetros equivalentes para la API externa
  ...
  // --- Combinar resultados ---
  const combinados = combinarResultados(resultadosLocales, externosFiltrados);
...
// 🎬 2. BÚSQUEDA POR CATEGORÍA — Combina peliculas.json + API externa
app.get("/peliculas/categoria/:genero", async (req, res) => {
    ...
    // Combinar y mezclar
    const combinados = shuffleArray(combinarResultados(resultadosLocales, resultadosExternos));
