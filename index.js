...
// ⏱️ Tiempo máximo de espera para la API externa (resiliencia silenciosa)
const EXTERNAL_API_TIMEOUT_MS = 10_000; // 10 segundos
 ...
 * Si la petición tarda más de EXTERNAL_API_TIMEOUT_MS, se cancela automáticamente.
 * Devuelve { ok: false } si hay error de red, timeout, 502 o 504.
 * El llamador decide qué hacer con la respuesta (nunca lanza excepción).
 */
async function fetchWithTimeout(url, options = {}, timeoutMs = EXTERNAL_API_TIMEOUT_MS) {
  ...
  try {
    ...
    clearTimeout(timer);
    // AbortError = timeout; FetchError = red caída
 ...
 * Si la API devuelve 502, 504, tarda más de 10s o falla de cualquier forma,
 * devuelve [] silenciosamente — el usuario nunca verá un error.
 */
async function fetchExternalSearch(params = {}) {
  try {
    ...
    return Array.isArray(data) ? data.map(p => ({ ...p, _fromExternalApi: true })) : [];
  } catch (err) {
 ...
 * Usa fetchWithTimeout para no esperar indefinidamente.
 * Si la API falla (502, 504, timeout), mantiene la caché anterior sin romper el servidor.
...
async function loadExternalApiMovies() {
  try {
    ...
    if (!resp.ok) {
      ...
      // Timeout ya fue logueado por fetchWithTimeout
...
// 🛡️ Resiliente: si la API externa falla, devuelve solo resultados locales sin error
...
app.get("/buscar", async (req, res) => {
  ...
  // --- Ejecutar búsqueda externa en paralelo con la local ---
...
// 🆕 ENDPOINT: Búsqueda por Categoría (Género) — combina peliculas.json + API externa
// 🛡️ Resiliente: si la API externa falla, devuelve solo resultados locales sin error
app.get("/peliculas/categoria/:genero", async (req, res) => {
    ...
    // Búsqueda local
    const localResultados = localPeliculas.filter(p =>
        (p.generos || "").toLowerCase().includes(generoBuscado)
    );

    // Búsqueda externa en paralelo
    const externalResults = await fetchExternalSearch({ q: generoRaw });
    ...
    if (combinado.length > 0) {
        return res.json({
            ...
            resultados: shuffleArray(combinado)
        });
    }
 ...
 * 🛡️ Resiliente: si la API externa falla (502, 504, timeout), responde con error claro
 * sin colgar el servidor. El timeout de 10s aplica aquí también.
 * GET /nuevo
 */
 ...
 * 🛡️ Resiliente: timeout de 10s con AbortController. Si falla, responde con error claro.
