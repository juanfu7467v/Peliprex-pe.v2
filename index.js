...
// ✅ FIX: Arranque inmediato en 0.0.0.0, carga externa en segundo plano, timeout en fetch, inactividad 5 min.
...
const EXTERNAL_API_URL = "https://peliprex-31wrsa.fly.dev/catalog";
const EXTERNAL_SEARCH_URL = "https://peliprex-31wrsa.fly.dev/search";
const EXTERNAL_API_REFRESH_MS = 2 * 60 * 60 * 1000; // Refrescar cada 2 horas

// ⏱️ Timeout máximo para cualquier petición a la API externa (5 segundos)
...
// Tiempo de inactividad para auto-cierre (5 minutos)
const TIEMPO_INACTIVIDAD = 5 * 60 * 1000;
const MS_IN_24_HOURS = 24 * 60 * 60 * 1000;

// ------------------- 2. CARGA SEGURA DEL JSON LOCAL -------------------
// Se carga de forma síncrona (rápido, archivo local) antes de arrancar el servidor.
let localPeliculas = [];
try {
...
// Catálogo combinado: comienza con las locales. Las externas se añaden en segundo plano al arrancar.
...
/**
 * fetch con timeout automático para evitar que el servidor quede colgado
 * esperando una API externa que no responde.
 */
...
/**
 * Combina resultados locales y externos evitando duplicados por título (normalizado).
 * Las películas del primer array tienen prioridad.
 */
function combinarResultados(primeros, segundos) {
  const titulosPrimeros = new Set(
    primeros.map(p => (p.titulo || "").toLowerCase().trim())
  );
  const segundosFiltrados = (segundos || []).filter(
    p => !titulosPrimeros.has((p.titulo || "").toLowerCase().trim())
  );
  return [...primeros, ...segundosFiltrados];
}

// ------------------- 4. FUNCIÓN DE BÚSQUEDA CON PRIORIDAD EXTERNA -------------------

/**
 * Realiza una petición a la API externa de búsqueda y devuelve los resultados.
 * Usa timeout de 5s. Nunca lanza excepción: en caso de error devuelve array vacío.
 */
async function buscarEnApiExterna(params = {}) {
  try {
    const url = new URL(EXTERNAL_SEARCH_URL);
    if (params.q)     url.searchParams.set("q", params.q);
    if (params.genre) url.searchParams.set("genre", params.genre);
    if (params.year)  url.searchParams.set("year", String(params.year));

    console.log(`📡 Consultando API externa: ${url.toString()}`);
    const resp = await fetchWithTimeout(url.toString());
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const data = await resp.json();
    // Normalizar: la API puede devolver array directo o { resultados: [...] }
    const lista = Array.isArray(data) ? data : (data.resultados || data.results || []);
    return lista.map(p => ({ ...p, _fromExternalApi: true }));
  } catch (err) {
    console.error("⚠️ Error al consultar API externa de búsqueda:", err.message);
    return [];
  }
}
 ...
 * ✅ Se llama en SEGUNDO PLANO después de que el servidor ya está escuchando.
 */
async function loadExternalApiMovies() {
  try {
    console.log("📡 Cargando películas desde API externa Peliprex...");
    const resp = await fetchWithTimeout(EXTERNAL_API_URL, {}, FETCH_TIMEOUT_MS);
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const data = await resp.json();
...
// ------------------- MIDDLEWARE DE ACTIVIDAD (ANTES DE RUTAS) -------------------
// Registra cada petición para el control de inactividad.
...
// Catálogo completo local + externo (en memoria)
app.get("/peliculas", (req, res) => res.json(peliculas));

// 🔎 1. BÚSQUEDA POR NOMBRE — Prioridad externa, fallback a local, respuesta combinada
app.get("/peliculas/:titulo", async (req, res) => {
  ...
  // Buscar en API externa primero (con timeout de 5s)
  const resultadosExternos = await buscarEnApiExterna({ q: tituloRaw });

  // Buscar también en peliculas.json (local, siempre disponible)
  const resultadosLocales = localPeliculas.filter(p =>
    (p.titulo || "").toLowerCase().includes(titulo)
  );

  // Combinar: externos primero, locales como complemento sin duplicados
...
// 🔎 3. BÚSQUEDA AVANZADA — Prioridad externa, fallback a local, respuesta combinada
app.get("/buscar", async (req, res) => {
  const { año, genero, idioma, desde, hasta, q } = req.query;

  // --- Búsqueda en API externa (primero, con timeout) ---
  const paramsExternos = {};
  if (q)      paramsExternos.q = q;
  if (genero) paramsExternos.genre = genero;
  if (año)    paramsExternos.year = año;
  // Si no hay q definido pero hay otros filtros, usar q=ALL para obtener resultados
  if (!paramsExternos.q && (genero || año || desde || hasta)) {
    paramsExternos.q = "ALL";
  }

  const resultadosExternos = await buscarEnApiExterna(paramsExternos);

  // Filtrar externos por rango de años si aplica (la API externa puede no soportarlo)
  let externosFiltrados = resultadosExternos;
  if (desde && hasta) {
    externosFiltrados = externosFiltrados.filter(
      p => parseInt(p.año) >= parseInt(desde) && parseInt(p.año) <= parseInt(hasta)
    );
  }

  // --- Búsqueda local (siempre disponible) ---
  let resultadosLocales = localPeliculas;
  ...
  // --- Combinar: externos primero, locales como complemento ---
  const combinados = combinarResultados(externosFiltrados, resultadosLocales);
...
// 🎬 2. BÚSQUEDA POR CATEGORÍA — Prioridad externa, fallback a local, respuesta combinada
app.get("/peliculas/categoria/:genero", async (req, res) => {
  ...
  // Búsqueda en API externa primero (con timeout)
  const resultadosExternos = await buscarEnApiExterna({ q: "ALL", genre: generoRaw });
  ...
  // Combinar externos + locales y mezclar aleatoriamente
  const combinados = shuffleArray(combinarResultados(resultadosExternos, resultadosLocales));
...
// 🆕 ENDPOINT: /nuevo — Catálogo directo desde API externa
/**
 * Retorna el catálogo completo de la API externa tal como llega.
 * Uso: GET /nuevo
 */
app.get("/nuevo", async (req, res) => {
  try {
    console.log("📡 Obteniendo catálogo desde API externa (/nuevo)...");
    const resp = await fetchWithTimeout(EXTERNAL_API_URL);
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const data = await resp.json();
    res.json(data);
  } catch (err) {
    console.error("❌ Error al obtener catálogo externo en /nuevo:", err.message);
    res.status(500).json({ error: "Error al obtener catálogo desde la API externa." });
  }
});
...
// ------------------- 6. app.listen NO BLOQUEANTE + CARGAS EN SEGUNDO PLANO -------------------
// ✅ FIX CRÍTICO: El servidor escucha PRIMERO en 0.0.0.0 para que Fly.io
// lo detecte de inmediato. Luego se cargan los datos externos en segundo plano.
// Esto elimina el error "instance refused connection" / "Aborted".
function startServer() {
  const PORT = process.env.PORT || 8080;

  // 1. ✅ Arrancar el servidor INMEDIATAMENTE en 0.0.0.0 (requerido por Fly.io)
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`✅ Servidor corriendo en http://0.0.0.0:${PORT}`);
    console.log("⏳ Iniciando carga de datos en segundo plano...");

    // 2. ✅ Sincronizar usuarios con GitHub EN SEGUNDO PLANO (sin bloquear)
    loadUsersDataFromGitHub()
      .then(() => console.log("✅ Sincronización de usuarios con GitHub completada."))
      .catch(err => console.error("⚠️ Error en sincronización GitHub:", err.message));

    // 3. ✅ Cargar catálogo externo EN SEGUNDO PLANO (sin bloquear)
    loadExternalApiMovies()
      .then(() => console.log("✅ Catálogo externo cargado en segundo plano."))
      .catch(err => console.error("⚠️ Error cargando catálogo externo:", err.message));

    // 4. ✅ Iniciar tareas periódicas DESPUÉS de que el servidor ya está en pie
    ...
    // Control de inactividad: respaldar y cerrar si no hay tráfico en 5 minutos.
    setInterval(async () => {
    ...
    }, 60 * 1000); // Chequear cada 1 minuto

    // Tarea de limpieza de actividad cada 24 horas.
    setInterval(() => {
