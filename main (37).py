...
# --- API EXTERNA DE CONTENIDO ---
EXTERNAL_API_BASE  = "https://peliprex-31wrsa.fly.dev"   # ✅ API externa unificada
...
# ---------------------------------------------------------------------------
# ✅ HELPER: Consultar API externa y normalizar resultados
# ---------------------------------------------------------------------------
async def _fetch_external_api(params: dict, timeout: float = 7.0) -> list:
    """
    Realiza una petición GET a EXTERNAL_API_BASE/search con los params dados.
    Devuelve una lista de items en el schema interno (título, imagen_url, etc.)
    compatible con _to_peliculas_json_schema. Nunca lanza excepción.
    """
    try:
        async with httpx.AsyncClient(timeout=timeout) as http:
            r = await http.get(f"{EXTERNAL_API_BASE}/search", params=params)
        if r.status_code != 200:
            return []
        data = r.json()
        # La API puede devolver una lista o un dict con una clave que contiene la lista
        if isinstance(data, list):
            items = data
        elif isinstance(data, dict):
            # Intentar claves comunes
            for key in ("results", "data", "items", "peliculas", "movies"):
        ...
        # Normalizar cada item al schema interno
        normalized = []
        for it in items:
            ...
            normalized.append({
                "titulo":            it.get("titulo") or it.get("title") or it.get("nombre") or "Película",
                "imagen_url":        it.get("imagen_url") or it.get("poster") or it.get("image") or "",
                "pelicula_url":      it.get("pelicula_url") or it.get("stream_url") or it.get("url") or "",
                "descripcion":       it.get("descripcion") or it.get("sinopsis") or it.get("description") or "",
                "fecha_lanzamiento": it.get("fecha_lanzamiento") or it.get("release_date") or "n/a",
                "duracion":          it.get("duracion") or it.get("runtime") or "n/a",
                "idioma_original":   it.get("idioma_original") or it.get("language") or "n/a",
                "popularidad":       it.get("popularidad") or it.get("popularity") or "n/a",
                "puntuacion":        it.get("puntuacion") or it.get("vote_average") or it.get("score") or "n/a",
                "generos":           it.get("generos") or it.get("genres") or it.get("genre") or "n/a",
                "año":               it.get("año") or it.get("year") or "n/a",
            })
        return normalized
    except Exception as _ext_err:
        print(f"⚠️  Error consultando API externa: {_ext_err}")
        return []


def _merge_with_external(local_results: list, external_results: list) -> list:
    """
    Une resultados locales con los de la API externa.
    Elimina duplicados por título normalizado, priorizando locales.
    """
    seen: set = set()
    merged: list = []
    for item in local_results + external_results:
        key = normalize_title(item.get("titulo") or item.get("title") or "")
...
async def search(
    ...
    try:
        ...
        # ✅ Consultar API externa en paralelo con los resultados de Telegram
        ext_params: dict = {}
        if effective_text:
            ext_params["q"] = effective_text
        elif effective_genre:
            ext_params["q"] = "ALL"
        if effective_genre:
            ext_params["genre"] = effective_genre
        if effective_year:
            ext_params["year"] = effective_year

        external_results: list = []
        if ext_params:
            try:
                external_results = await asyncio.wait_for(
                    _fetch_external_api(ext_params), timeout=7.0
                )
                print(f"🌐 API externa → {len(external_results)} resultado(s)")
            except asyncio.TimeoutError:
                print("⚠️  API externa timeout en /search")
        ...
        if not final_results and effective_text:
            ...
            if yt_results:
                ...
                # ✅ Combinar con resultados externos antes de devolver
                local_schema  = _to_peliculas_json_schema(enriched)
                ext_schema    = _to_peliculas_json_schema(external_results)
                return _merge_with_external(local_schema, ext_schema)
        ...
        # ✅ Combinar resultados locales enriquecidos con resultados externos
        local_schema = _to_peliculas_json_schema(enriched)
        ext_schema   = _to_peliculas_json_schema(external_results)
        combined     = _merge_with_external(local_schema, ext_schema)
        print(f"✅ /search combinado: {len(local_schema)} locales + {len(ext_schema)} externos = {len(combined)} únicos")
        return combined
...
# ---------------------------------------------------------------------------
# ENDPOINT /peliculas/{nombre}   — búsqueda por título combinada
# ---------------------------------------------------------------------------
@app.get("/peliculas/{nombre}")
async def peliculas_por_nombre(nombre: str, request: Request):
    """
    Busca películas/series por nombre.
    Combina resultados de Telegram (via /search interno) y la API externa.
    """
    try:
        ...
        # Buscar en Telegram en paralelo con la API externa
        telegram_task  = asyncio.create_task(
            search(q=nombre, year=None, genre=None, language=None,
                   desde=None, hasta=None, canal=None)
        )
        external_task  = asyncio.create_task(
            _fetch_external_api({"q": nombre})
        )

        telegram_raw, external_raw = await asyncio.gather(
            telegram_task, external_task, return_exceptions=True
        )

        telegram_items = telegram_raw if isinstance(telegram_raw, list) else []
        external_items = _to_peliculas_json_schema(
            external_raw if isinstance(external_raw, list) else []
        )

        combined = _merge_with_external(telegram_items, external_items)
        print(f"🎬 /peliculas/{nombre}: {len(telegram_items)} Telegram + {len(external_items)} externos = {len(combined)} únicos")
        return combined

    except HTTPException:
        raise
    except Exception as e:
        print(f"❌ Error en /peliculas/{nombre}: {e}")
        return {"error": str(e)}


# ---------------------------------------------------------------------------
# ENDPOINT /peliculas/categoria/{genero}  — búsqueda por categoría combinada
# ---------------------------------------------------------------------------
@app.get("/peliculas/categoria/{genero}")
async def peliculas_por_categoria(genero: str, request: Request):
    """
    Busca películas/series por categoría/género.
    Combina resultados de Telegram y la API externa.
    """
    try:
        ...
        # Buscar en Telegram y API externa en paralelo
        telegram_task = asyncio.create_task(
            search(q=None, year=None, genre=genero, language=None,
                   desde=None, hasta=None, canal=None)
        )
        external_task = asyncio.create_task(
            _fetch_external_api({"q": "ALL", "genre": genero})
        )
        ...
        print(f"🎭 /peliculas/categoria/{genero}: {len(telegram_items)} Telegram + {len(external_items)} externos = {len(combined)} únicos")
        return combined

    except HTTPException:
        raise
    except Exception as e:
        print(f"❌ Error en /peliculas/categoria/{genero}: {e}")
        return {"error": str(e)}


# ---------------------------------------------------------------------------
# ENDPOINT /buscar  — búsqueda avanzada combinada con múltiples filtros
# ---------------------------------------------------------------------------
@app.get("/buscar")
async def buscar(
    q:      str | None = Query(None, description="Texto de búsqueda (título, palabras clave)"),
    año:    str | None = Query(None, description="Año exacto (ej: 2024)"),
    genero: str | None = Query(None, description="Género o categoría (ej: Accion, Terror, Anime)"),
    desde:  int | None = Query(None, description="Año mínimo (ej: 2010)"),
    hasta:  int | None = Query(None, description="Año máximo (ej: 2024)"),
):
    """
    Búsqueda avanzada combinada: Telegram + API externa.
    Soporta filtros por texto, año, género y rango de años.
    """
    try:
        has_any = any([q, año, genero, desde, hasta])
        if not has_any:
            raise HTTPException(
                status_code=400,
                detail="Se requiere al menos un parámetro: q, año, genero, desde, hasta"
            )

        # Construir parámetros para API externa
        ext_params: dict = {}
        if q:
            ext_params["q"] = q.strip()
        elif genero:
            ext_params["q"] = "ALL"
        if genero:
            ext_params["genre"] = genero.strip()
        if año:
            ext_params["year"] = año.strip()

        # Lanzar Telegram y API externa en paralelo
        telegram_task = asyncio.create_task(
            search(q=q, year=año, genre=genero, language=None,
                   desde=desde, hasta=hasta, canal=None)
        )
        external_task = asyncio.create_task(
            _fetch_external_api(ext_params) if ext_params else asyncio.sleep(0)
        )
        ...
        combined = _merge_with_external(telegram_items, external_items)
        print(f"🔍 /buscar: {len(telegram_items)} Telegram + {len(external_items)} externos = {len(combined)} únicos")
        return combined
...
# ---------------------------------------------------------------------------
# ENDPOINT /nuevo  — contenido nuevo desde la API externa
# ---------------------------------------------------------------------------
@app.get("/nuevo")
async def nuevo():
    """
    Devuelve el catálogo más reciente de la API externa (peliprex-31wrsa.fly.dev/catalog).
    """
    try:
        async with httpx.AsyncClient(timeout=10.0) as http:
            r = await http.get(f"{EXTERNAL_API_BASE}/catalog")
        if r.status_code != 200:
            raise HTTPException(status_code=502, detail="API externa no disponible")
        data = r.json()
        print(f"🆕 /nuevo: {len(data) if isinstance(data, list) else '?'} items desde API externa")
        return data
    except HTTPException:
        raise
    except Exception as e:
